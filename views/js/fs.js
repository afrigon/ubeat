class Util { // eslint-disable-line max-lines
...
class FS {
    ...
    static logError (err) {
        console.error(`FrigStudio Error line(${err.line}:${err.column}): ${err.message}`); // eslint-disable-line no-console
    }
}
...
class GradientFader extends Component {
    constructor (options) {
        ...
        const defaultAngle = 135,
            defaultDurationPerColor = 10000; // ms
...
class AutoScrollAnimator extends Component {
    ...
    setVisibility (element, isVisible) {
        if (!isVisible) {
            element.setAttribute("data-animation-name", getComputedStyle(element).getPropertyValue("animation-name") || "none");
    ...
    getElementOffsetToTop (element, offset) {
        try {
            ...
            let el = element,
                elementOffset = element.offsetTop + usableOffset;
            while (el) {
                el = el.offsetParent;
                elementOffset += el
                    ? el.offsetTop
    ...
    isElementOnScreen (element) {
        const elementOffset = element.getAttribute("data-offset") || 0,
            elementTop = this.getElementOffsetToTop(element, elementOffset),
            screenHeight = window.innerHeight || document.documentElement.clientHeight,
            topEdge = this.options.scrollContainer.scrollTop || window.pageYOffset,
            bottomEdge = topEdge + Math.min(window.document.documentElement.clientHeight, screenHeight); // eslint-disable-line sort-vars
    ...
    scrollCallback () {
        if (ScrollAnimator.hasScroll && ScrollAnimator.animators.length) {
            ...
            const screenHeight = window.innerHeight || document.documentElement.clientHeight,
                bottomEdge = window.pageYOffset + screenHeight; // eslint-disable-line sort-vars

            ScrollAnimator.animators.forEach((animator) => {
                const bottomPoint = bottomEdge - animator.options.startOffset * screenHeight,
                    elementOffset = ScrollAnimator.getElementOffsetToTop(animator.element),
                    topPoint = bottomPoint - animator.options.duration;
                ...
                if (topPoint > elementOffset) {
                    return animator.animate(0);
                }

                if (bottomPoint < elementOffset) {
                    return animator.animate(100);
                }
...
class ColorScrollAnimator extends ScrollAnimator {
    ...
    animate (percent) {
        const blueValue = Math.round((100 - percent) * (this.endColor.blue - this.startColor.blue) / 100 + this.startColor.blue),
            greenValue = Math.round((100 - percent) * (this.endColor.green - this.startColor.green) / 100 + this.startColor.green),
            redValue = Math.round((100 - percent) * (this.endColor.red - this.startColor.red) / 100 + this.startColor.red);
